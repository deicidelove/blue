...
public class WorkDao {
	...
	public BlueNeedWork findBySid(int sid) {
		Map<String, Object> paramMap = Maps.newHashMap();
		StringBuilder sql = new StringBuilder("SELECT * FROM tb_blue_needWork WHERE sid = :sid");
	...
	public int updateWork(BlueNeedWork needWork) {
		String sql = "UPDATE tb_blue_needWork SET title =:title,needNum =:needNum,education =:education,experience = :experience,wages = :wages,work_time = :work_time,"
				+ "work_address =:work_address,description=:description,requirement=:requirement,fringe_benefits:=fringe_benefits WHERE sid =:sid";
		Map<String, Object> paramMap = Maps.newHashMap();
