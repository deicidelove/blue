...
(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarTitle_text.text(year + '年' + month +'月') ;
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        if (returnDateStr(new Date()) === allDay_str) {
          $(this).attr('class', 'item flex-1 day_li').siblings().hide();
          var n = $(this).index();
            var obj = $(this).siblings();
            obj.eq(n-3).show().attr('class', 'item flex-1');
            obj.eq(n-2).show().attr('class', 'item flex-1');
            obj.eq(n-1).show().attr('class', 'item flex-1');
            obj.eq(n).show().attr('class', 'item flex-1');
            obj.eq(n+1).show().attr('class', 'item flex-1');
            obj.eq(n+2).show().attr('class', 'item flex-1');
          ...
          //var data1 = ['','','','','',20170901,20170902,20170903,20170904];
          ////for( var j = 0; j > data1.length;j++){
          //    console.log(data1[i],9999)
          //    console.log(allDay_str,8888)
          //    if(data1[i] == allDay_str){
    ...
    renderDOM: function () { // 渲染DOM
      ...
      this.$calendar_week = $('<ul class="calendar-week box"></ul>');
      this.$calendar_date = $('<ul class="calendar-date box"></ul>');
      ...
      var _titleStr ='';
                      ...
                      //'</div>';
      var _weekStr = '<li class="item flex-1">日</li>'+
                      '<li class="item flex-1">一</li>'+
                      '<li class="item flex-1">二</li>'+
                      '<li class="item flex-1">三</li>'+
                      '<li class="item flex-1">四</li>'+
                      '<li class="item flex-1">五</li>'+
                      '<li class="item flex-1">六</li>';
      var _dateStr = '';
      ...
      if (this.opts.ifSwitch) {
        this.$arrow_prev.bind('click', function () {
          var _date = dateObj.getDate();
                //console.log(_date.getMonth()-1,9999999)
  ...
  function changingStr(fDate) { // 字符串转日期
    ...
    //if(index == 1){
        ...
        $('.sign_bg').on('click',function(){
            $(this).hide();
            $('.sign_div').hide();
            $('#sign').css({'background':'#b5b5b5','color':'#fff'}).text('今日已签到');
        });
